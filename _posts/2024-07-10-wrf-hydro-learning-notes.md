...
目前WRF-Hydro支持的陆面模型为Noah和Noah-MP，未来预计会支持其他。目前模型需要的气候驱动变量要求，采用耦合方式运行时，这些数据根据指定的频率由气候模型提供。当单独运行WRF-Hydro时，需要按照grided input time series提供。
...
之后WRF-Hydro根据离地表最近的饱和土层顶部深度计算地下水位深度。举例，在WRF-Hydro中2m的土柱，最小要有4个土壤层，但是这并不是一个严格的要求。额外的离散可以提高对随时间变化的地下水位高度的分辨率，用户可以在附录A3、A4和A5描述的model namelist中修改土壤层数和厚度。
